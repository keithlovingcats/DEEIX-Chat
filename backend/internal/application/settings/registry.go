...
var settingSpecs = []settingSpec{
	...
	{Namespace: "auth", Key: "third_party_login_enabled", ValueType: "bool", Default: "false", Description: "是否启用第三方登录入口",
		Validate: boolValue(), Apply: applyField(func(c *config.Config) *bool { return &c.ThirdPartyLoginEnabled }, toBool)},
	{Namespace: "auth", Key: "email_registration_enabled", ValueType: "bool", Default: "false", Description: "是否允许邮箱注册",
	...
	{Namespace: "billing", Key: "prepaid_amount_usd", ValueType: "string", Default: "0", Description: "每个付费调用预留的风险预算(美元)，0表示按剩余槽位动态分配可用预算并保底0.1美元，最多20个并发调用",

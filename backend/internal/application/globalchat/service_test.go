...
func TestSendTextValidation(t *testing.T) {
	...
	if _, err := service.SendText(context.Background(), 0, "sess-1", "dev-1", "hi"); !errors.Is(err, ErrInvalidMessage) {
		t.Fatalf("SendText() with zero user error = %v, want ErrInvalidMessage", err)
	}
	if _, err := service.SendText(context.Background(), 7, "sess-1", "dev-1", "   "); !errors.Is(err, ErrInvalidMessage) {
		t.Fatalf("SendText() blank content error = %v, want ErrInvalidMessage", err)
	}
	if _, err := service.SendText(context.Background(), 7, "sess-1", "dev-1", strings.Repeat("字", maxTextContentLength+1)); !errors.Is(err, ErrInvalidMessage) {
...
func TestSendTextSnapshotsUserAndBroadcasts(t *testing.T) {
	...
	item, err := service.SendText(context.Background(), 7, "sess-1", "dev-1", "大家好 😀")
	if err != nil {
	...
	if item.SessionID != "sess-1" || item.DeviceID != "dev-1" {
		t.Fatalf("SendText() session/device = %q/%q, want sess-1/dev-1", item.SessionID, item.DeviceID)
	}
...
func TestSendTextFallbackDisplayName(t *testing.T) {
	...
	item, err := service.SendText(context.Background(), 8, "sess-1", "dev-1", "hi")
	if err != nil {
		t.Fatalf("SendText() error = %v", err)
	}
...
func TestSendImageValidation(t *testing.T) {
	...
	if _, err := service.SendImage(context.Background(), 7, "sess-1", "dev-1", ""); !errors.Is(err, ErrInvalidMessage) {
		t.Fatalf("SendImage() empty fileID error = %v, want ErrInvalidMessage", err)
	}
	repo.imageErr = repository.ErrNotFound
	if _, err := service.SendImage(context.Background(), 7, "sess-1", "dev-1", "missing"); !errors.Is(err, ErrImageFileInvalid) {
		t.Fatalf("SendImage() missing file error = %v, want ErrImageFileInvalid", err)
	}
	repo.imageErr = nil
	repo.imageFile.MimeType = "application/pdf"
	if _, err := service.SendImage(context.Background(), 7, "sess-1", "dev-1", "f1"); !errors.Is(err, ErrImageFileInvalid) {
...
func TestSendImageCreatesMessageWithFileReference(t *testing.T) {
	...
	item, err := service.SendImage(context.Background(), 7, "sess-1", "dev-1", "img-1")
	if err != nil {

...
func TestDescribeUpstreamRequestFailureExposesUpstreamSummary(t *testing.T) {
	err := errors.Join(appconversation.ErrUpstreamRequestFailed, &llm.UpstreamError{
		StatusCode: 401,
		Message:    "google authentication failed",
	})

	mapped := describeUpstreamRequestFailure(err)
	if mapped.Status != http.StatusBadGateway {
...
func TestDescribeUpstreamRequestFailurePinsUpstreamCode(t *testing.T) {
	// 上游消息以 "not found" 结尾等场景下，启发式推断会把中文摘要 slug 化，
	// 钉稳定码避免前端词典 miss。
	err := errors.Join(appconversation.ErrUpstreamRequestFailed, &llm.UpstreamError{
		StatusCode: 404,

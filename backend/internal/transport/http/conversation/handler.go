...
func describeUpstreamRequestFailure(err error) response.Description {
	...
	// 信息量与落库 error_message 一致，不被 DescribeCode 泛化为标准文案；
	// 错误码已钉稳定值，中文摘要不会被启发式 slug 化成不可读错误码。
	return response.Description{Status: http.StatusBadGateway, Code: code, Message: mapClientErrorMessage(err)}
}
...
// mapClientErrorMessage 把上游失败的对外文案映射为客户端可读摘要（jun 定制透传通道）。

...
- **多模型讨论**：讨论 = 逐轮辩论、轮内并行——2-20 个模型 N 轮辩论（最少 2 轮；第 1 轮全员盲测：透传原始问题、互不可见，消除首模型锚定，首条 default 请求的 `message_created` 一到即并行 fan-out 其余发言；第 N 轮只审阅第 N-1 轮及以前的轮次快照、轮内互不可见且并行，消除轮内序列特权；终稿凭全量 transcript 合成）。防内容性带偏四件套：用户问题以 `<user_question>` 标签定界、transcript 发言以 `<turn>` 标签定界，两类定界内的字面标签（含用户问题中粘贴的外部文本）一律转义为 HTML 实体（防伪造结构边界/间接注入）；互审轮 transcript 为自己历史发言标 `Speaker N (you)`（立场演进需显式锚点；终稿不标，保持全匿名裁决）；超长发言截断对齐换行并修补未闭合代码围栏（栈匹配 ```/~~~，未截断发言同样修补）。编排器 `use-chat-discussion.ts` 轮间 barrier、轮内 `Promise.all`（每发言独立 run，峰值并发与 `MAX_CONCURRENT_RUNS` 上限对齐）；讨论发言随消息落库标记 `chat_messages.discussion_meta_json`（前端随请求透传 `discussionMeta`），刷新后凭 meta 重建聚合面板（`DiscussionPanel`，组内消息不走 `ModelBranchTabs`）。⚠️ 关键通道（讨论 prompt 进生成上下文的两级配合，均有单测锁定）：讨论发言走 retry 复用原 user 消息、content 不落库——`service_message_preparation.go` 的 reuseUserMessage 分支仅当 `DiscussionMeta == nil` 才回填原 user content；`service_message_route_attempt.go` 的 `OverrideReusedUserContent` 再以 content **替换**链尾 user 文本（不追加，避免与原问题重复及连续 user 消息违反 Anthropic 协议）。开关/轮数是会话内内存态，不持久化；终稿容错：总结模型失败先同模型重试 1 次，仍失败按参与序换「本次讨论有成功发言」的参与者接替（每候选一次重试），全部候选耗尽或达总尝试上限（5 次）才 error，失败尝试以同 index 多条 final 消息落库（面板可见，渲染取最新成功稿；同 index 按 serverMessageID / createdAt 打破平局）

...
export function planChatSubmission(input: {
  ...
  /** 多模型并行 fan-out：请求锚点来自 message_created 服务端事件，豁免本地互斥校验且不跟随切换分支。 */
  ...
  // 多模型 fan-out 不跟随切换分支（兄弟并行生成，保持用户当前视图）。
  const shouldFollowSubmittedBranch =
    !input.programmaticFanOut &&
    (!queuedSubmission ||
      branchRunIsVisible(
        targetBranchScope,
        clientRunID,
  ...
  if (
    ...
    (!input.programmaticFanOut && !queuedSubmission && input.uploading) ||
    (!concurrentBranchRun && !input.programmaticFanOut && targetConversationHasActiveStream)
  ) {

...
export const GENERATION_CANCEL_SETTLEMENT_TIMEOUT_MS = 25_000;
...
export function useChatStopMessage({
  ...
  reload: () => void;
  /** 多模型并行：停止只作用于当前可见 run，其余 sibling 仍在运行时提示（避免误以为全停）。 */
  onParallelRunsRemaining?: (stoppedRunID: string) => void;
}) {
  return React.useCallback(() => {
    ...
    if (!active.accessToken) {
      ...
      onParallelRunsRemaining?.(active.runID);
      return true;
    }

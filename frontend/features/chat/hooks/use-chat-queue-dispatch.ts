...
import { MAX_DISCUSSION_MODELS } from "@/features/chat/hooks/use-chat-discussion";
...
export function useChatQueueDispatch({
  ...
  /** 多模型讨论配置（会话内内存态）；入队时开启的组合出队后仍以讨论形式发出。 */
  multiModelDiscussion?: { enabled: boolean; rounds: number };
  /** 讨论编排器的发送入口（use-chat-discussion 的 sendWithDiscussion ref）。 */
  sendWithDiscussionRef?: React.RefObject<((input: {
    content: string;
  ...
  React.useEffect(() => {
    ...
    resumeGenerationActive,
    submitMessage,
    multiModelDiscussion?.enabled,
    multiModelDiscussion?.rounds,
    sendWithDiscussionRef,
    visibleBranchScopePath,
    visibleMessages,
    activeStreamsRef,
